...
namespace dazhuankuai.classes
{
    public class Frame
    {
        ...
        public void GoLeft()
        {
            if (canmove && buffle.left >= BuffleMoveDistance + Pos.Left)
            {
        ...
        public void GoRight()
        {
            if (canmove && buffle.left <= Pos.Right - BuffleWidth - BuffleMoveDistance)
            {
                buffle.left += BuffleMoveDistance;
                canmove = false;
            }
        ...
        static readonly float MoveChangeFactor = 2.0f;
